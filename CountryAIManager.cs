...
public class CountryAIManager : MonoBehaviour
{
    ...
    public void Process(CountrySettings country)
    {
                ...
                foreach (CountrySettings _country in GetBorderingCountiesWithRegion(region))
                {
                        ...
                        FindCountriesRelation(country, _country).war) // Если граничащая страна воюет с нами
                    {
                        if (!warBorderingRegions.Contains(region))
                        {
                            ...
                            if (ReferencesManager.Instance.gameSettings.difficultyValue.value == "INSANE" || ReferencesManager.Instance.gameSettings.difficultyValue.value == "HARDCORE")
                            {
        ...
        foreach (RegionManager region in country.myRegions)
        {
            ...
            {
                if (region.GetDivision()._movePoints > 0)
                {
                    foreach (RegionManager neighbour_region in GetNeiboursOfRegion(region))
                    {
                        if (!neighbour_region.hasArmy)
                        {
                            ...
                            {
                                GameObject division = new GameObject();
                                UnitMovement division2 = new UnitMovement();
                                foreach (Transform item in region.transform)
                                {
                                    if (item.GetComponent<UnitMovement>())
                                    {
                                        division = item.gameObject;
                                        division2 = item.GetComponent<UnitMovement>();
                                    }
                                }
                                division2._movePoints--;
                                division2.firstMove = false;
                                division.transform.SetParent(neighbour_region.transform, false);
                                if (ReferencesManager.Instance.gameSettings.onlineGame)
                                {
                                    Multiplayer.Instance.MoveUnit(region._id, neighbour_region._id);
                                }
                                else
                                {
                                    //division2.UpdateInfo();
                                }
