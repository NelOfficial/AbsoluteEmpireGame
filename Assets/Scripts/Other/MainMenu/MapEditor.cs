...
public class MapEditor : MonoBehaviour
{
    ...
    [SerializeField] TMP_InputField yearInputField;
    ...
    public void SaveOldModName(string name)
    {
        _oldModName = name;
    }
    ...
    public void CompileMod()
    {
        ...
        SaveOldModName(nameInputField.text);

        currentModText = $"NAME = [{nameInputField.text}];ALLOW_EVENTS = [{eventsState}];YEAR = [{yearInputField.text}];";
        CompileCountries();
        CompileRegions();
        ...
        if (string.IsNullOrEmpty(ReferencesManager.Instance.gameSettings.editingModString.value))
        {
            if (!string.IsNullOrEmpty(currentModText) && !string.IsNullOrWhiteSpace(currentModText))
            {
                if (!string.IsNullOrEmpty(nameInputField.text) && !string.IsNullOrWhiteSpace(nameInputField.text))
                {
                    ...
                    {
                        if (!string.IsNullOrEmpty(yearInputField.text) && !string.IsNullOrWhiteSpace(yearInputField.text))
                        {
    ...
    private void CompileEvents()
    {
        ...
        for (int i = 0; i < _eventCreatorManager.modEvents.Count; i++)
        {
            try
            {
                File.Delete(Path.Combine(Application.persistentDataPath, "localmods",
                    nameInputField.text, "events", $"{_eventCreatorManager.modEvents[i].id}",
                    $"{_eventCreatorManager.modEvents[i].id}.AEEvent"));
            }
            catch (Exception) {}
            ...
            try
            {
                CheckEventImage(_eventCreatorManager.modEvents[i]);
            }
            catch (Exception ex)
            {
                Debug.LogError(ex);
    ...
    private void CheckEventImage(EventCreatorManager.ModEvent _event)
    {
        string oldPath = _event.imagePath;

        string newPath = Path.Combine(Application.persistentDataPath, "localMods",
            nameInputField.text, "events", $"{_event.id}", $"{_event.id}.jpg");
