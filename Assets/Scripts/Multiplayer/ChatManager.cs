...
public class ChatManager : MonoBehaviour, IChatClientListener
{
    ...
    public void OnSubscribed(string[] channels, bool[] results)
    {
        for (int i = 0; i < channels.Length; i++)
        {
            ...
            spawnedChatItem.GetComponent<ChatListItem>()._text = $"Вы подключены к {channels[i]}";
            spawnedChatItem.GetComponent<ChatListItem>()._author = "[Система]";
    ...
    public void OnUnsubscribed(string[] channels)
    {
        for (int i = 0; i < channels.Length; i++)
        {
            ...
            spawnedChatItem.GetComponent<ChatListItem>()._text = $"Вы отключены от {channels[i]}";
            spawnedChatItem.GetComponent<ChatListItem>()._author = "[Система]";
            spawnedChatItem.GetComponent<ChatListItem>().SetUp();
    ...
    public void OnUserSubscribed(string channel, string user)
    {
        ...
        spawnedChatItem.GetComponent<ChatListItem>()._text = $"Игрок {user} подключился к {channel}";
        spawnedChatItem.GetComponent<ChatListItem>()._author = "[Система]";
        spawnedChatItem.GetComponent<ChatListItem>().SetUp();
    ...
    public void OnUserUnsubscribed(string channel, string user)
    {
        ...
        spawnedChatItem.transform.localScale = new Vector3(1, 1, 1);
        spawnedChatItem.GetComponent<ChatListItem>()._text = $"Игрок {user} отключися от {channel}";
        spawnedChatItem.GetComponent<ChatListItem>()._author = "[Система]";
        spawnedChatItem.GetComponent<ChatListItem>().SetUp();
