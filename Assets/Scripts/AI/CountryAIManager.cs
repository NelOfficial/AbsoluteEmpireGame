    ...
    public void Process(CountrySettings country, int mode)
    {
        ...
        foreach (CountrySettings countryOther in ReferencesManager.Instance.countryManager.countries)
        {
            if (countryOther.country._id != country.country._id)
            {
                ...
                if (mode == 1)
                {
                    int relationship = ReferencesManager.Instance.diplomatyUI.FindCountriesRelation(countryOther, country).relationship;
                    if (relationship < -50)
                    {
                        if (ReferencesManager.Instance.diplomatyUI.FindCountriesRelation(countryOther, country).pact)
                        {
                    ...
                    else if (country.myRegions.Count / 2 > countryOther.myRegions.Count)
                    {
                        if (ReferencesManager.Instance.diplomatyUI.FindCountriesRelation(countryOther, country).pact)
                        {
                            SendOffer("Разрыв пакта о ненападении", country, countryOther);
