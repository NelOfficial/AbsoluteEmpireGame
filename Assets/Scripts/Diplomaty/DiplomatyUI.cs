...
public class DiplomatyUI : MonoBehaviour
{
    ...
    public void ApplySelectionOfCountries(string a)
    {
        ...
        if (data[0] == "ask_for_war")
        {
            ...
            if (senderToReceiver.union || receiverToSender.union)
            {
                random += Random.Range(20, 35);
            }

            if (senderToReceiver.trade || receiverToSender.trade)
            {
                random += Random.Range(1, 5);
            }

            if (receiver.vassalOf = sender)
            {
                random += Random.Range(40, 90);
            }

            if (senderToReceiver.pact || receiverToSender.pact)
            {
                random += Random.Range(1, 10);
            }

            if (senderToReceiver.right || receiverToSender.right)
            {
                random += Random.Range(1, 10);
            }
